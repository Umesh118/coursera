...
export class PromotionService {
  ...
  getFeaturedPromotion(): Observable<Promotion> {
    return this.http.get<Promotion>(baseURL + 'promotions?featured=true').pipe(map((promotions: any) => promotions[0])).pipe(catchError(this.processHTTPMsgService.handleError));
  }
}
